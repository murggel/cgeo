...
public class NumberFilterViewHolder<F extends NumberRangeGeocacheFilter<Float>> extends BaseFilterViewHolder<F> {
    ...
    NumberFilterViewHolder(float minValue, float maxValue) {
    ...
    public F createFilterFromView() {
        ...
        filter.setMinMaxRange(range.left, range.right, minValue, maxValue, value -> Math.round(value * granularity) / granularity);
        return filter;
    }
}
